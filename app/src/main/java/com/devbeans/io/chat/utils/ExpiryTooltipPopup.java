 ...
 */
public class ExpiryTooltipPopup extends PopupWindow {
    ...
    private ExpiryTooltipPopup(@NonNull View anchor,
                               int rawPosition,
                               @NonNull String text,
        ...
        super(LayoutInflater.from(anchor.getContext()).inflate(R.layout.expiry_tooltip, null),
        ...
        switch (rawPosition) {
            case POSITION_ABOVE:
                arrow = getContentView().findViewById(R.id.expiry_tooltip_arrow_bottom);
                break;
            case POSITION_BELOW:
                arrow = getContentView().findViewById(R.id.expiry_tooltip_arrow_top);
                break;
            case POSITION_START:
                arrow = getContentView().findViewById(R.id.expiry_tooltip_arrow_end);
                break;
            case POSITION_END:
                arrow = getContentView().findViewById(R.id.expiry_tooltip_arrow_start);
                break;
            default:
        ...
        TextView textView = getContentView().findViewById(R.id.expiry_tooltip_text);
        ...
        View bubble = getContentView().findViewById(R.id.expiry_tooltip_bubble);
        View bubbleMain = getContentView().findViewById(R.id.expiry_tooltip_bubble_main);

        if (backgroundTint == 0) {
//            bubble.getBackground().setColorFilter(ContextCompat.getColor(anchor.getContext(), R.color.core_white), PorterDuff.Mode.MULTIPLY);
            bubbleMain.getBackground().setColorFilter(ContextCompat.getColor(anchor.getContext(), R.color.core_white), PorterDuff.Mode.MULTIPLY);
            arrow.setColorFilter(ContextCompat.getColor(anchor.getContext(), R.color.core_white), PorterDuff.Mode.MULTIPLY);
        } else {
//            bubble.getBackground().setColorFilter(backgroundTint, PorterDuff.Mode.MULTIPLY);
            bubbleMain.getBackground().setColorFilter(backgroundTint, PorterDuff.Mode.MULTIPLY);
            arrow.setColorFilter(backgroundTint, PorterDuff.Mode.MULTIPLY);
        }

        if (iconGlideModel != null) {
            ImageView iconView = getContentView().findViewById(R.id.expiry_tooltip_icon);
...
//        setOutsideTouchable(true);
    ...
    private void show() {
        ...
        getContentView().measure(View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.EXACTLY),
    ...
    private static void setArrowHorizontalPosition(@NonNull View arrow, @NonNull View anchor) {
        ...
        arrow.setTranslationX((float) anchorCenterX - (float) arrowCenterX);
    }
    ...
    private static void setArrowVerticalPosition(@NonNull View arrow, @NonNull View anchor) {
        ...
        arrow.setTranslationY((float) anchorCenterY - (float) arrowCenterY);
    }
    ...
    public static class Builder {
        ...
        private ExpiryTooltipPopup tooltip;

        private Builder(@NonNull View anchor) {
            this.anchor = anchor;
        }

        public ExpiryTooltipPopup getToolTip() {
            return tooltip;
        }
        ...
        public ExpiryTooltipPopup show(int position) {
            String text = anchor.getContext().getString(textResId);
            if (tooltip != null && tooltip.isShowing()) {
                tooltip.dismiss();
            }
            tooltip = new ExpiryTooltipPopup(anchor, position, text, backgroundTint, textColor, iconGlideModel, dismissListener);

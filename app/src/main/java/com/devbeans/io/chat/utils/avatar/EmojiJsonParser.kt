...
//    private val OBJECT_MAPPER = ObjectMapper()
    ...
    fun parse(body: InputStream, uriFactory: UriFactory): Result<ParsedEmojiData> {
        return try {
            Result.success(buildEmojiSourceFromNode( uriFactory))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }

    private fun buildEmojiSourceFromNode( uriFactory: UriFactory): ParsedEmojiData {
        val format: String = ""
        val obsolete: List<ObsoleteEmoji> = emptyList()
        val dataPages: List<EmojiPageModel> = emptyList()
        val jumboPages: Map<String, String> = emptyMap()
        val displayPages: List<EmojiPageModel> = emptyList()
//        val metrics: EmojiMetrics =
        val densities: List<String> = emptyList()

        return ParsedEmojiData( densities, format, displayPages, dataPages, jumboPages, obsolete)
    }

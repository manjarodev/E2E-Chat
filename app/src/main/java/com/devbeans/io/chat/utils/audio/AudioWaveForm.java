...
import com.devbeans.io.chat.utils.MediaInput;
import com.devbeans.io.chat.utils.ThreadUtil;
import com.devbeans.io.chat.utils.concurrent.SerialExecutor;
...
public final class AudioWaveForm {
    ...
    private final File slide;
    private long[] wave;
    private int[] waveSamples;

    public AudioWaveForm(@NonNull Context context, @NonNull File slide) {
        this.context = context.getApplicationContext();
        this.slide = slide;
    }

    private static final LruCache<String, AudioFileInfo> WAVE_FORM_CACHE = new LruCache<>(200);
    private static final Executor AUDIO_DECODER_EXECUTOR = new SerialExecutor(ChatExecutors.BOUNDED);

    @AnyThread
    public void getWaveForm(@NonNull Consumer<AudioFileInfo> onSuccess, @NonNull Runnable onFailure) {
        Uri uri = Uri.parse(slide.getPath());
...
//            }
    ...
    @RequiresApi(api = 23)
    private @NonNull
    AudioFileInfo generateWaveForm(@NonNull Uri uri) throws IOException {
        try (MediaInput dataSource = new MediaInput.UriMediaInput(context, uri)) {
            wave = new long[BAR_COUNT];
            waveSamples = new int[BAR_COUNT];
           ...
           handleWaves(format,codec,extractor,totalDurationUs);


            return new AudioFileInfo(totalDurationUs, getBytes(waveSamples,wave));
        }
    }

    private void handleWaves(MediaFormat format, MediaCodec codec, MediaExtractor extractor, long totalDurationUs) {
    ...
    private byte[] getBytes(int[] waveSamples, long[] wave) {

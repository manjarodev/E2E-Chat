...
final class ChatCameraXModule {
    ...
    private final ChatCameraView mCameraView;
    final AtomicBoolean mVideoIsRecording = new AtomicBoolean(false);
    private ChatCameraView.CaptureMode mCaptureMode = ChatCameraView.CaptureMode.IMAGE;
    private long mMaxVideoDuration = ChatCameraView.INDEFINITE_VIDEO_DURATION;
    private long mMaxVideoSize = ChatCameraView.INDEFINITE_VIDEO_SIZE;
    ...
    ChatCameraXModule(ChatCameraView view, Consumer<Throwable> errorConsumer) {
    ...
    void bindToLifecycleAfterViewMeasured() {
        ...
        if (getCaptureMode() == ChatCameraView.CaptureMode.IMAGE) {
            mCamera = mCameraProvider.bindToLifecycle(mCurrentLifecycle, cameraSelector,
                    mImageCapture,
                    mPreview);
        } else if (getCaptureMode() == ChatCameraView.CaptureMode.VIDEO) {
            mCamera = mCameraProvider.bindToLifecycle(mCurrentLifecycle, cameraSelector,
                    mVideoCapture,
                    mPreview);
        } else {
    ...
    public void takePicture(Executor executor, OnImageCapturedCallback callback) {
        ...
        if (getCaptureMode() == ChatCameraView.CaptureMode.VIDEO) {
    ...
    @NonNull
    public ChatCameraView.CaptureMode getCaptureMode() {
        return mCaptureMode;
    }

    public void setCaptureMode(@NonNull ChatCameraView.CaptureMode captureMode) {

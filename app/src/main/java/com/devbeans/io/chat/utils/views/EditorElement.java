...
import com.devbeans.io.chat.utils.EditorFlags;
import com.devbeans.io.chat.utils.MatrixUtils;
import com.devbeans.io.chat.utils.ParcelUtils;
import com.devbeans.io.chat.utils.Renderer;
import com.devbeans.io.chat.utils.RendererContext;
import com.devbeans.io.chat.utils.editor.AlphaAnimation;
import com.devbeans.io.chat.utils.editor.AnimationMatrix;
...
/**
 * An image consists of a tree of {@link EditorFlags}s.
 * <p>
 * Each element has some persisted state:
 * - An optional {@link com.devbeans.io.chat.utils.Renderer} so that it can draw itself.
...
public final class EditorElement implements Parcelable {
    ...
    public EditorElement findElement(@NonNull EditorElement toFind, @NonNull Matrix viewMatrix, @NonNull Matrix outInverseModelMatrix) {
        return findElement(viewMatrix, outInverseModelMatrix, (element, inverseMatrix) -> toFind == element);
    }

    public EditorElement findElementAt(float x, float y, @NonNull Matrix viewModelMatrix, @NonNull Matrix outInverseModelMatrix) {
    ...
    public void forAllInTree(@NonNull PerElementFunction function) {
        function.apply(this);
    ...
    public void deleteChild(@NonNull EditorElement editorElement, @Nullable Runnable invalidate) {
    ...
    public void addDeletedChildFadingOut(@NonNull EditorElement fromElement, @Nullable Runnable invalidate) {
        deletedChildren.add(fromElement);
        fromElement.animateFadeOut(invalidate);
    }

    public void animateFadeOut(@Nullable Runnable invalidate) {
        alphaAnimation = AlphaAnimation.animate(1, 0, invalidate);
    }

    public void animateFadeIn(@Nullable Runnable invalidate) {
    ...
    public @Nullable EditorElement parentOf(@NonNull EditorElement element) {
    ...
    public void rollbackEditorMatrix(@Nullable Runnable invalidate) {
    ...
    public void animateFrom(@NonNull Matrix oldMatrix, @Nullable Runnable invalidate) {

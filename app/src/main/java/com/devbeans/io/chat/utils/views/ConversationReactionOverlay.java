...
import com.devbeans.io.chat.models.SelectedConversationModel;
...
public final class ConversationReactionOverlay extends FrameLayout {
    ...
    private Conversation conversationRecipient;
    private Payload messageRecord;
...
//        customEmojiIndex = emojiViews.length - 1;
    ...
    public void show(@NonNull Activity activity,
                     @NonNull Conversation conversationRecipient,
                     @NonNull Payload conversationMessage,
...
//        this.messageRecord = conversationMessage.getMessageRecord();
        ...
        this.payload = conversationMessage;
...
//        setupSelectedEmoji();
        ...
        ViewKt.doOnLayout(this, v -> {
            showAfterLayout(activity, lastSeenDownPoint, isMessageOnLeft);
    ...
    private void showAfterLayout(@NonNull Activity activity,
        ...
        contextMenu = new ConversationContextMenu(dropdownAnchor, getMenuActionItems());
        ...
        if (isMessageOnLeft) {
            ...
            scrubberX = (float) getWidth() - (float) scrubberWidth - (float) scrubberHorizontalMargin;
        }
        ...
        if (isWideLayout) {
            ...
            contextMenu.show((int) offsetX, (int) Math.min(backgroundView.getY(), (float) overlayHeight - (float) contextMenu.getMaxHeight()));
        } else {
    ...
    private void updateToolbarShade(@NonNull Activity activity) {
        ...
        View bannerContainer = activity.findViewById(R.id.rootView);
    ...
    private int getInputPanelHeight(@NonNull Activity activity) {
        View bottomPanel = activity.findViewById(R.id.layout_message);
        View emojiDrawer = activity.findViewById(R.id.iv_emoji);
...
//        int barColor = ContextCompat.getColor(getContext(), R.color.transparent_color);

        if (window != null) {
//            originalStatusBarColor = window.getStatusBarColor();
            WindowUtil.setStatusBarColor(window, originalStatusBarColor);

//            originalNavigationBarColor = window.getNavigationBarColor();
            WindowUtil.setNavigationBarColor(window, originalNavigationBarColor);
        }
...
//        segmentSize = emojiStripViewBounds.width() / (float) emojiViews.length;
    ...
    public boolean applyTouchEvent(@NonNull MotionEvent motionEvent) {
        ...
        handleOverlayState(motionEvent);
    ...
    private void handleOverlayState(MotionEvent motionEvent) {
...
//    String getOldEmoji(@NonNull MessageRecord messageRecord) {
...
//    }
    ...
    /**
     * This method fetches the menu action items based on the payload properties.
     * It checks the type of the payload and its properties and adds the appropriate ActionItem(s) to the items list.
     * It ensures that the payload is not deleted, not confidential, and is outgoing for 'text' type.
     * It also sets the visibility of backgroundView and foregroundView to INVISIBLE.
     *
     * @return a list of ActionItems
     */
    private @NonNull List<ActionItem> getMenuActionItems() {
        items = new ArrayList<>();
        if (payload == null) {
            return items;
        }

        handleOtherTextTypePayload();

        handleSimpleOptions();

        addActionItem(R.drawable.ic_delete_tinted_24, R.string.conversation_selection__menu_delete, Action.DELETE);

        handleNonTextTypePayload();
    ...
    /**
     * Handles the actions for a payload of type 'text'
     */
    private void handleOtherTextTypePayload() {
        // Checks if payload is 'text' type, not confidential, not deleted and is outgoing
        if (payload.getType().equalsIgnoreCase("text") && !payload.isConfidential() && !payload.isDeleted() && selectedConversationModel.isOutgoing()) {
            addActionItem(R.drawable.ic_edit_tinted_24, R.string.conversation_selection__menu_edit, Action.EDIT);
        }
    }

    /**
     * Handles the actions for a payload of other types
     */
    private void handleNonTextTypePayload() {
        // Check if the payload type is not 'text' and it's not deleted
        if (!payload.getType().equalsIgnoreCase("text") && !payload.isDeleted()) {
            // will be used for info showing
            // addActionItem(R.drawable.ic_info_tinted_24, R.string.conversation_selection__menu_message_details, Action.VIEW_INFO);
        }
    }

    /**
     * Sets the visibility of backgroundView and foregroundView to INVISIBLE
     */
    private void setViewVisibility() {
        backgroundView.setVisibility(View.INVISIBLE);
        foregroundView.setVisibility(View.INVISIBLE);
    }

    /**
     * Adds an ActionItem to the 'items' list
     *
     * @param drawableId Drawable resource Id
     * @param stringId   String resource Id
     * @param action     Action to be performed
     */
    private void addActionItem(int drawableId, int stringId, Action action) {
        items.add(new ActionItem(drawableId, getResources().getString(stringId), () -> handleActionItemClicked(action)));
    }


    /**
     * This method is used to handle the simple options based on the payload properties.
     * It checks the type of the payload and its properties and adds the appropriate ActionItem(s) to the items list.
     * It ensures that the payload is not deleted, not confidential, not view-once, and can share forward.
     */
    private void handleSimpleOptions() {
        // Check if payload meets the criteria for further processing
        if (!isPayloadProcessable()) {
            return;
        }

        // Add relevant actions based on payload type
        switch (payload.getType().toLowerCase()) {
            case "text":
                handleTextTypePayload();
                break;

            case "image":
                handleImageTypePayload();
                break;

            default:
                handleDefaultTypePayload();
                break;
        }
    }

    /**
     * Checks if the payload can be processed further
     *
     * @return true if the payload meets the criteria, false otherwise
     */
    private boolean isPayloadProcessable() {
        return !payload.isCanShareForward() && !payload.isConfidential() && !payload.isViewOnce() && !payload.isDeleted();
    }

    /**
     * Handles the actions for a payload of type 'text'
     */
    private void handleTextTypePayload() {
        // Checks if payload is not deleted and filePath is not null
        if (payload.getFilePath() != null) {
            addActionItem(R.drawable.ic_forward_24_tinted, R.string.conversation_selection__menu_forward, Action.FORWARD);
        }

        addActionItem(R.drawable.ic_copy_24_tinted, R.string.conversation_selection__menu_copy, Action.COPY);
    }

    /**
     * Handles the actions for a payload of type 'image'
     */
    private void handleImageTypePayload() {
     ...
     * Handles the actions for a payload of other types
     */
    private void handleDefaultTypePayload() {
        // Check if the filePath is not null
        if (payload.getFilePath() != null) {
            addActionItem(R.drawable.ic_save_24_tinted, R.string.conversation_selection__menu_save, Action.DOWNLOAD);
            addActionItem(R.drawable.ic_share_tinted_24, R.string.conversation_selection__menu_share, Action.SHARE);
        }
    }
    ...
    private void handleActionItemClicked(@NonNull Action action) {
        ...
        //moved this out side of the OnHideListener because it was giving delay and also context was being canceld due to it
...
//                .toList();
        ...
        @SuppressLint("RestrictedApi") Animator backgroundRevealAnim = AnimatorInflaterCompat.loadAnimator(getContext(), android.R.animator.fade_in);
...
//        reveals.add(backgroundRevealAnim);
        ...
        @SuppressLint("RestrictedApi") Animator selectedRevealAnim = AnimatorInflaterCompat.loadAnimator(getContext(), android.R.animator.fade_in);
...
//        revealAnimatorSet.playTogether(reveals);
    ...
    private @NonNull
    AnimatorSet newHideAnimatorSet() {
...
//    private @NonNull
//    List<Animator> newHideAnimators() {

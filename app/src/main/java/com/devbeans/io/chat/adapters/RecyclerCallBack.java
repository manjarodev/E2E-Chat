...
import com.devbeans.io.chat.callBacks.OnActivityCallBack;
...
public class RecyclerCallBack implements RecyclerView.OnItemTouchListener, OnActivityCallBack {
...
//                    vibe.vibrate(100); // do we really need to add vibrate service
    ...
    public RecyclerCallBack(Activity a, RecyclerView recyclerView) {
        this.act = a;
...
//        mSwipeListener = listener;
        ...
        rView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                /**
                 * This will ensure that this RecyclerCallBack is paused during recycler view scrolling.
    ...
    public RecyclerCallBack setClickable(OnRowClickListener listener) {
        this.clickable = true;
        this.mRowClickListener = listener;
        return this;
    }

    public RecyclerCallBack setClickable(boolean clickable) {
        this.clickable = clickable;
        return this;
    }

    public RecyclerCallBack setLongClickable(boolean vibrate, OnRowLongClickListener listener) {
        this.longClickable = true;
        this.mRowLongClickListener = listener;
        this.longClickVibrate = vibrate;
        return this;
    }
    public RecyclerCallBack setLongClickable(boolean longClickable) {
        this.longClickable = longClickable;
        return this;
    }

    public RecyclerCallBack setIndependentViews(Integer... viewIds) {
        this.independentViews = new ArrayList<>(Arrays.asList(viewIds));
        return this;
    }

    public RecyclerCallBack setUnClickableRows(Integer... rows) {
        this.unClickableRows = new ArrayList<>(Arrays.asList(rows));
        return this;
    }

    public RecyclerCallBack setIgnoredViewTypes(Integer... viewTypes) {
    ...
    public RecyclerCallBack setSwipeable(int foregroundID, int backgroundID, int backgroundID1, OnSwipeOptionsClickListener listener) {
        this.check=true;
        this.swipeable = true;
    ...
    public RecyclerCallBack setSwipeable(int foregroundID, int backgroundID, OnSwipeOptionsClickListener listener) {
        this.check=false;
        this.swipeable = true;
        if (fgViewID != 0 && foregroundID != fgViewID)
            throw new IllegalArgumentException("foregroundID does not match previously set ID");
    ...
    public RecyclerCallBack setSwipeable(boolean value) {
    ...
    public RecyclerCallBack setSwipeOptionViews(Integer... viewIds) {
        this.optionViews = new ArrayList<>(Arrays.asList(viewIds));
        return this;
    }

    public RecyclerCallBack setUnSwipeableRows(Integer... rows) {
    ...
    public void openSwipeOptions(int position) {
        ...
        {
            if (act.findViewById(bgViewID) != null)
                bgWidthLeft = act.findViewById(bgViewIDLeft).getWidth();
            heightOutsideRView = screenHeight - rView.getHeight();
        }
        ...
        bgViewLeft.setMinimumHeight(fgView.getHeight());
    ...
    private void animateFG(View downView, Animation animateType, long duration) {
        ...
        else if (animateType == Animation.LEFT) {
//            ObjectAnimator translateAnimator = ObjectAnimator.ofFloat(
//                    fgView, View.TRANSLATION_X, +bgWidth/2);
//            translateAnimator.setDuration(duration);
//            translateAnimator.setInterpolator(new DecelerateInterpolator(1.5f));
//            translateAnimator.start();
//            animateFadeViews(downView, 1f, duration);
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (touchedView != null) {
                    ...
                    if (swipeable) {
                        ...
                        fgView = touchedView.findViewById(fgViewID);
                        bgView = touchedView.findViewById(bgViewID);
                        if (!Objects.equals(bgViewLeft,null)) {
                            bgViewLeft = touchedView.findViewById(bgViewIDLeft);
                            bgViewLeft.setMinimumHeight(fgView.getHeight());
                        }
//                        bgView.getLayoutParams().height = fgView.getHeight();
            ...
            case MotionEvent.ACTION_UP: {
                ...
                else if (Math.abs(mFinalDelta) > bgWidthLeft / 2 && isFgSwiping)
                {
                    swipedLeftProper = mFinalDelta < 0;
                    swipedRightProper = mFinalDelta > 0;
                }
                ...
                // if swiped right properly and options menu isn't already visible, animate the foreground to the right
                    ...
                    if (check) {
                        ...
                        ++mDismissAnimationRefCount;
                        //TODO - speed
                        animateFG(touchedView, Animation.LEFT, ANIMATION_STANDARD);
                        bgVisible = true;
                        bgVisibleView = fgView;
                        bgVisiblePosition = downPosition;
                    }
                ...
                // it's open position (spring effect)
                    ...
                    final View tempBgView = bgViewLeft;
                    animateFG(touchedView, Animation.CLOSE, ANIMATION_STANDARD, new OnSwipeListener() {
                        @Override
                        public void onSwipeOptionsClosed() {
                            if (tempBgView != null)
...
//                        }
                    ...
                    // if fg is being swiped right
                        ...
                        if (bgVisible) {
                            float translateAmount = (deltaX - mSwipingSlop) - bgWidthLeft;
                            ...
                            if (fadeViews != null) {
                                for (int viewID : fadeViews) {
                                    touchedView.findViewById(viewID).setAlpha(1 - (Math.abs(translateAmount) / bgWidthLeft));
                                }
                            }
                        }
...
//                        }
    ...
    public interface RecyclerTouchListenerHelper {
        void setOnActivityTouchListener(OnActivityCallBack listener);
    }

...
import com.devbeans.io.chat.utils.audio.AudioWaveForm;
import com.devbeans.io.chat.utils.components.voice.VoiceNotePlaybackState;
import com.devbeans.io.chat.utils.events.PartProgressEvent;
...
//    }
    ...
    public void setAudio(final @NonNull File audio,
        ...
        if (seekBar instanceof WaveFormSeekBarView) {
            if (audioSlide != null && !Objects.equals(Uri.parse(audioSlide.getPath()), Uri.parse(audio.getPath()))) {
...
//        }
        ...
        handleSeekBarView(forceHideDuration,audio);
    }

    private void handleSeekBarView(boolean forceHideDuration, File audio) {
        if (seekBar instanceof WaveFormSeekBarView) {
            WaveFormSeekBarView waveFormView = (WaveFormSeekBarView) seekBar;
...
//    }
    ...
    private void onStart(@NonNull Uri uri, boolean statePlaying, boolean autoReset) {
        if (!isTarget(uri) || !statePlaying) {
            if (hasAudioUri()) {
                onStop(Uri.parse(audioSlide.getPath()), autoReset);
            }
    ...
    private boolean isTarget(@NonNull Uri uri) {
        return hasAudioUri() && Objects.equals(uri, Uri.parse(audioSlide.getPath()));
    }

    private boolean hasAudioUri() {
        return audioSlide != null && Uri.parse(audioSlide.getPath()) != null;
    }
...
//        this.downloadButton.setColorFilter(foregroundTint, PorterDuff.Mode.SRC_IN);
    ...
    private void startLottieAnimation(int direction) {
        ...
        playPauseButton.setSpeed((float) direction * 2);
    ...
    public void stopPlaybackAndReset() {
        if (audioSlide == null || Uri.parse(audioSlide.getPath()) == null) return;

        if (callbacks != null) {
            callbacks.onStopAndReset(Uri.parse(audioSlide.getPath()));
            rewind();
        }
    }
        ...
        public void onClick(View v) {
            ...
            if (callbacks != null) {
                if (lottieDirection == REVERSE) {
                    callbacks.onPlay(Uri.parse(audioSlide.getPath()), getProgress());
                } else {
                    callbacks.onPause(Uri.parse(audioSlide.getPath()));
                }
            }
        }
    }
...
//    }
    ...
    private class SeekBarModifiedListener implements SeekBar.OnSeekBarChangeListener {
        ...
        public synchronized void onStopTrackingTouch(SeekBar seekBar) {
            ...
            if (callbacks != null) {
                if (wasPlaying) {
                    callbacks.onSeekTo(Uri.parse(audioSlide.getPath()), getProgress());
                } else {
    ...
    public void onEventAsync(final PartProgressEvent event) {
        if (audioSlide != null && circleProgress != null && event.attachment.equals(Uri.parse(audioSlide.getPath()))) {

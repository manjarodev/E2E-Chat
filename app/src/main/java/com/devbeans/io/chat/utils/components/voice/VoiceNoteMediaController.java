...
import com.devbeans.io.chat.utils.DefaultValueLiveData;
...
//                String                           threadRecipient = message.getThreadRecipientId();
...
//
//                return Transformations.map( displayName -> Optional.of(
...
//            }
    ...
    private void startPlayback(@NonNull Uri audioSlideUri, long messageId, long threadId, double progress, boolean singlePlayback) {
        ...
        MediaControllerCompat mediaControllerCompat = getMediaController();
        ...
        if (isCurrentTrack(audioSlideUri)) {
            long duration = mediaControllerCompat.getMetadata().getLong(MediaMetadataCompat.METADATA_KEY_DURATION);

            mediaControllerCompat.getTransportControls().seekTo((long) (duration * progress));
            ...
            mediaControllerCompat.getTransportControls().playFromUri(audioSlideUri, extras);
        }
    }
    ...
    public void setPlaybackSpeed(@NonNull Uri audioSlideUri, float playbackSpeed) {
        ...
        if (isCurrentTrack(audioSlideUri)) {
            ...
            mediaControllerCompat.sendCommand(VoiceNotePlaybackService.ACTION_NEXT_PLAYBACK_SPEED, bundle, null);
        }
    }
    ...
    private boolean isCurrentTrack(@NonNull Uri uri) {
        ...
        metadataCompat = mediaControllerCompat.getMetadata();
    ...
    private void notifyProgressEventHandler() {
        ...
        if (progressEventHandler == null && activity != null) {
            progressEventHandler = new ProgressEventHandler(mediaControllerCompat, voiceNotePlaybackState);
    ...
    VoiceNotePlaybackState extractStateFromMetadata(@NonNull MediaControllerCompat mediaController,
...
//        }
    ...
    private static @Nullable
    VoiceNotePlaybackState constructPlaybackState(@NonNull MediaControllerCompat mediaController,
                                                  @Nullable VoiceNotePlaybackState previousState) {
        MediaMetadataCompat mediaMetadataCompat = mediaController.getMetadata();
...
//        long        messageId         = -1L;
//        String senderId          = RecipientId.UNKNOWN;

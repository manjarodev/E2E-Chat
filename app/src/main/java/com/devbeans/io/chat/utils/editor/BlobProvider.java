...
import com.devbeans.io.chat.utils.IOFunction;
import com.devbeans.io.chat.utils.ModernDecryptingPartInputStream;
import com.devbeans.io.chat.utils.StreamUtil;
...
public class BlobProvider {
    ...
    public synchronized @NonNull InputStream getStream(@NonNull Context context, @NonNull Uri uri, long position) throws IOException {
        ...
        return getBlobRepresentation(context,
                ...
                file -> ModernDecryptingPartInputStream.createFor("null",
                        file,
                        position));
    }
...
//        File             outputFile       = new File(getOrCreateDirectory(context, directory), buildFileName(blobSpec.id));
        ...
        File tempFile = new File(MainApp.Companion.getImagesPathSent(), "fileName");
        FileOutputStream outputStream = new FileOutputStream(tempFile);
//        OutputStream          = ModernEncryptingPartOutputStream.createFor("attachmentSecret", outputFile, true).second;

        ChatExecutors.UNBOUNDED.execute(() -> {
            try {
    ...
    @WorkerThread
    private synchronized @NonNull Uri writeAudioBlobSpecToDiskAsync(@NonNull Context context,
                                                                    @NonNull BlobSpec blobSpec,
                                                                    @Nullable SuccessListener successListener,
                                                                    @Nullable ErrorListener errorListener)
            throws IOException
...
//        AttachmentSecret attachmentSecret = AttachmentSecretProvider.getInstance(context).getOrCreateAttachmentSecret();
        String           directory        = getDirectory(blobSpec.getStorageType());
        File             outputFile       = new File(getOrCreateDirectory(context, directory), buildFileName(blobSpec.id));
//        OutputStream     outputStream     = ModernEncryptingPartOutputStream.createFor(outputFile, true).second;
    ...
    public class BlobBuilder {
         ...
         */
        @WorkerThread
        public Uri createDataForDraftAttachmentAsync(@NonNull Context context,
                                                     @Nullable SuccessListener successListener,
                                                     @Nullable ErrorListener errorListener)
                throws IOException
        {
            return writeAudioBlobSpecToDiskAsync(context, buildBlobSpec(StorageType.ATTACHMENT_DRAFT), successListener, errorListener);
        }
    }

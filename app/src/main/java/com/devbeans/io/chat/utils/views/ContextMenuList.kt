...
import com.devbeans.io.chat.adapters.MappingAdapter
...
class ContextMenuList(recyclerView: RecyclerView, onItemClick: () -> Unit) {
    ...
    private val mappingAdapter = MappingAdapter().apply {
        registerFactory(
            DisplayItem::class.java,
            LayoutFactory({ ItemViewHolder(it, onItemClick) }, R.layout.signal_context_menu_item)
        )
    }
    ...
    private class ItemViewHolder(
        ...
        override fun bind(model: DisplayItem) {
            ...
            icon.visibility = if (model.item.iconRes == 0) View.GONE else View.VISIBLE

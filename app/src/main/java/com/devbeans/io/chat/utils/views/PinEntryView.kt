...
class PinEntryView @JvmOverloads constructor(
    ...
    private var mLineGradientStartColor: Int = 0
    private var mLineGradientEndColor: Int = 0
    ...
    init {
        ...
        mLineGradientStartColor = a.getColor(R.styleable.PinView_lineGradientStartColor, -1)
        mLineGradientEndColor = a.getColor(R.styleable.PinView_lineGradientEndColor, -1)
        ...
        mCursorColor = a.getColor(R.styleable.PinView_cursorColorPin, currentTextColor)
        mCursorWidth = a.getDimensionPixelSize(
            R.styleable.PinView_cursorWidthPin,
    ...
    private fun drawPinView(canvas: Canvas) {
        ...
        for (i in 0 until mPinItemCount) {
            ...
            if (mViewType == VIEW_TYPE_RECTANGLE) {
                if (mLineGradientStartColor != -1 && mLineGradientEndColor != -1) {
                    mPaint.shader = LinearGradient(0f, 0f, width.toFloat(), 0f, mLineGradientStartColor, mLineGradientEndColor, Shader.TileMode.CLAMP)
                } else {
                ...
                // Use the gradient if available, otherwise fallback to the color
                if (mLineGradientStartColor != -1 && mLineGradientEndColor != -1) {
                    mPaint.shader = LinearGradient(0f, 0f, width.toFloat(), 0f, mLineGradientStartColor, mLineGradientEndColor, Shader.TileMode.CLAMP)
                } else {
                    mPaint.color = getLineColorForState(*HIGHLIGHT_STATES)
...
//            mPaint.color = getLineColorForState(*HIGHLIGHT_STATES)
    ...
    companion object {
        ...
        private fun isPasswordInputType(inputType: Int): Boolean {
            ...
            return (variation == EditorInfo.TYPE_CLASS_TEXT or EditorInfo.TYPE_TEXT_VARIATION_PASSWORD || variation == EditorInfo.TYPE_CLASS_TEXT or EditorInfo.TYPE_TEXT_VARIATION_WEB_PASSWORD || variation == EditorInfo.TYPE_CLASS_NUMBER) or (variation == EditorInfo.TYPE_NUMBER_VARIATION_PASSWORD)
        }
    }
}
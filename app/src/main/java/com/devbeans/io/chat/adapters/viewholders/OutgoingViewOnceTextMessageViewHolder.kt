...
class OutgoingViewOnceTextMessageViewHolder(itemView: View, payload: Any?) :
    OutcomingTextMessageViewHolder<Messages>(itemView, payload) {
    var tvTime: TextView? = null

    var edited: TextView? = null
    val tag =
        com.devbeans.io.chat.utils.logging.Log.tag(OutgoingViewOnceTextMessageViewHolder::class.java)
...
//            messageText!!.text = message.payload.data
            disappearIcon!!.visibility =
                if (message.payload.isExpire) VISIBLE else GONE
            ...
            when {
                (message.payload.isDeleted) -> {
                    messageText!!.setTextColor(appContext!!.resources.getColor(R.color.black_variation_50))
                }
                else -> {
        ...
        messageText?.setCompoundDrawablesWithIntrinsicBounds(
            if (message.payload.isOpened) R.drawable.ic_view_once_opened_sender else R.drawable.ic_view_once_sender,
            0,
            0,
            0
        )

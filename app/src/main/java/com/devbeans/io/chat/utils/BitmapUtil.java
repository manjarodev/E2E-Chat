...
public class BitmapUtil {
            ...
            try {
                ...
                if (bytes.length > maxImageSize) {
                    if (sizeAttempt <= MAX_IMAGE_HALF_SCALES) {
                        ...
                        throw new Exception("Unable to scale image below " + bytes.length + " bytes.");
                    }
                }

                if (bytes.length <= 0) {
                    throw new Exception("Decoding failed. Bitmap has a length of " + bytes.length + " bytes.");
                }
    ...
    CompressFormat getCompressFormatForContentType(@Nullable String contentType) {
        if (contentType == null) return CompressFormat.JPEG;

        switch (contentType) {
            case MediaUtil.IMAGE_JPEG:
                return CompressFormat.JPEG;
            case MediaUtil.IMAGE_PNG:
                return CompressFormat.PNG;
            case MediaUtil.IMAGE_WEBP:
    ...
    private static BitmapFactory.Options getImageDimensions(InputStream inputStream)
        ...
        if (options.outWidth == -1 || options.outHeight == -1) {
            throw new Exception("Failed to decode image dimensions: " + options.outWidth + ", " + options.outHeight);
        }
    ...
    public static @Nullable
    byte[] toWebPByteArray(@Nullable Bitmap bitmap) {
        if (bitmap == null) return null;
        ByteArrayOutputStream stream = new ByteArrayOutputStream();

...
import com.devbeans.io.chat.utils.Debouncer
...
import com.devbeans.io.chat.utils.editor.HSVColorSlider
import com.devbeans.io.chat.utils.editor.HSVColorSlider.getColor
import com.devbeans.io.chat.utils.editor.HSVColorSlider.setColor
...
class ImageEditorHudV2 @JvmOverloads constructor(
    ...
    init {
        inflate(context, R.layout.v2_chat_media_image_editor_hud, this)
    }
    ...
    public fun initializeViews() {
        ...
        cropAspectLockButton.setOnClickListener {
            ...
            if (listener?.isCropAspectLocked == true) {
                cropAspectLockButton.setImageResource(R.drawable.ic_crop_lock_chat)
            } else {
                cropAspectLockButton.setImageResource(R.drawable.ic_crop_unlock_chat)
            }
        }
    ...
    private fun setupWidthSeekBar() {
        ...
        widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                ...
                when (currentMode) {
                    Mode.DRAW -> SettingsValues.imageEditorValues().setMarkerPercentage(progress)
                    Mode.BLUR -> SettingsValues.imageEditorValues().setBlurPercentage(progress)
                    Mode.HIGHLIGHT -> SettingsValues.imageEditorValues().setHighlighterPercentage(progress)
                    else -> Unit
                }
            }
    ...
    fun showBlurHudTooltip() {
        TooltipPopup.forTarget(blurButton)
            .setText(R.string.image_editor_draw_anywhere_to_blur)
    ...
    private fun presentModeDraw() {
        ...
        brushToggle.setImageResource(R.drawable.ic_draw_chat_white)
        widthSeekBar.progress = SettingsValues.imageEditorValues().getMarkerPercentage()
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
    ...
    private fun presentModeHighlight() {
        drawButton.isSelected = true
        brushToggle.setImageResource(R.drawable.ic_marker_chat)
        widthSeekBar.progress = SettingsValues.imageEditorValues().getHighlighterPercentage()
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
            inSet = drawButtonRow + drawTools,
    ...
    private fun presentModeBlur() {
        ...
        widthSeekBar.progress = SettingsValues.imageEditorValues().getBlurPercentage()
        listener?.onColorChange(getActiveColor())
        updateColorIndicator()
        animateModeChange(
    ...
    companion object {
        ...
        private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
            Mode.DRAW to SettingsValues.imageEditorValues().getMarkerWidthRange(),
            Mode.HIGHLIGHT to SettingsValues.imageEditorValues().getHighlighterWidthRange(),
            Mode.BLUR to SettingsValues.imageEditorValues().getBlurWidthRange()
        )

...
class OutgoingTextMessageViewHolder(itemView: View, payload: Any?) :
    ...
    private fun handleText(message: Messages) {
        when {
            (!message.payload.isConfidential) -> {
            ...
            (message.payload.isConfidential) -> {
                arrayOf(
                    messageText,
                    textTimeAndTickLayout,
                    forwardLayout
                ).forEach {
                    it?.visibility =
                        GONE
                }
                arrayOf(messageTextCon, textTimeAndTickLayoutCon).forEach {
                    it?.visibility = VISIBLE
                }
                tvTimeCon!!.text = DateTime.getFormattedTimeOnly(message.payload.createdAt)
    ...
    private fun init(itemView: View) {
        messageText = itemView.findViewById<View>(R.id.messageText0) as EmojiTextView
        ...
        messageText!!.setEmojiSize(appContext!!.resources.getDimensionPixelSize(emojiSize))
        tvTime = itemView.findViewById(R.id.text_textview_time)

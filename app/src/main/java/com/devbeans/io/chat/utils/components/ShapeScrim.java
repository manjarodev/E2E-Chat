...
public class ShapeScrim extends View {
    ...
    public ShapeScrim(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.canvasColor = Color.parseColor("#B3202020");
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        float left = ((float) getMeasuredWidth() / 2) - drawRadius;
        float top = ((float) getMeasuredHeight() / 2) - drawRadius;
    ...
    private void drawCircle(Canvas canvas, float radius, Paint eraser) {
        canvas.drawCircle((float) getWidth() / 2, (float) getHeight() / 2, radius, eraser);
    }
